...
float hdr_heatmap_lilium_fade_in(const float Y, const float currentStop, const float normaliseTo) {
  return (Y - currentStop) / (normaliseTo - currentStop);
}

float hdr_heatmap_lilium_fade_out(const float Y, const float currentStop, const float normaliseTo) {
